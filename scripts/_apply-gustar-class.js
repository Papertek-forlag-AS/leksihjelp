...
const DSTS = [
  path.join(__dirname, '..', 'extension', 'data', 'es.json'),
  path.join(__dirname, '..', 'tests', 'fixtures', 'vocab', 'es.json'),
];
...
function applyTo(dstPath) {
  ...
  for (const [srcId, srcEntry] of Object.entries(srcVerb)) {
    ...
    if (dstId) {
      ...
      // Not present — copy the lexicon entry over.
  ...
  if (JSON.stringify(flagged) !== JSON.stringify(expected)) {
    throw new Error(`${dstPath}: mismatch. flagged=${flagged.join(',')} expected=${expected.join(',')}`);
  }
  console.log(`[ok] ${path.relative(process.cwd(), dstPath)}: marked ${marked}, added ${added}, pedagogy.gustar_class set`);
}

for (const dst of DSTS) applyTo(dst);
