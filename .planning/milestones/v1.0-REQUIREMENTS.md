# Requirements Archive: v1.0 Spell-Check & Prediction Quality
...
- [x] **SC-03**: Cross-standard token detection — when a word is valid only in the sister standard (e.g., `ikkje` typed inside an NB document), flag with a register-aware suggestion. Reverses earlier dialect-tolerance framing per user domain policy (NB/NN are two distinct official written standards; memory `project_nb_nn_no_mixing.md`). *(Phase 4 landed the seam rail — vocab.sisterValidWords; Phase 05.1 Gap D reversed semantics via new `nb-dialect-mix.js` priority 35 with CROSS_DIALECT_MAP as authoritative fire-gate; typo rules no longer early-exit into sister-valid lookup. Chrome smoke-test 2026-04-21 PASS across cross-dialect sweep.)*
...
- [x] **SC-05**: Production-quality særskriving detection — the `sarskriving` rule passes the regression fixture's target precision/recall thresholds (thresholds set during INFRA-02)
...
| SC-03 | Phase 4 + Phase 05.1 | Complete (2026-04-21) |
| SC-04 | Phase 4 | Complete |
| SC-05 | Phase 4 | Complete |
...
*Last updated: 2026-04-21 after v1.0 milestone audit — SC-03 reversed to flag-not-tolerate per user domain policy (memory `project_nb_nn_no_mixing.md`) + checkbox ticked; SC-05 checkbox ticked + trace row set Complete (THRESHOLDS P=1.000 R=1.000 at 0.92/0.95 gate in check-fixtures.js).*
