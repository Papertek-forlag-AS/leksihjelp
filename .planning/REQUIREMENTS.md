...
- [ ] **HYG-01**: UAT walkthrough template exists at `.planning/uat/TEMPLATE-walkthrough.md` with mandatory pre-flight evidence block (extension version string, IDB revision, default-preset profile, browser+version, `chrome://extensions` reload timestamp), defects-observed section, and target-browsers list
- [ ] **HYG-02**: Finding template exists at `.planning/uat/TEMPLATE-finding.md` formalizing the F-id pattern (severity, sync-status, regression-fixture-id) — promotes the implicit F1/F2/F36-1 convention from v3.1 Phase 34/35/36 to repo standard
