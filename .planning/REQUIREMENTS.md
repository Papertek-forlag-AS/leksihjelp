...
- [x] **INFRA-07**: Document-state two-pass runner (`kind: 'document'` rule type) with explicit invalidation protocol — consumed by all DOC-* rules
...
- [x] **INFRA-10**: New release gate `check-stateful-rule-invalidation` — paired self-test for Phase 13 document-state; plants a broken invalidation, gate must fire
...
- [x] **DOC-03**: NB bokmål/riksmål mixing warn when paragraph contains riksmål forms (`boken`, `efter`, `sne`) alongside bokmål; `BOKMAL_RIKSMAL_MAP` extension to v1.0 cross-dialect infra
- [x] **DOC-04**: NN a-infinitiv / e-infinitiv mixing warn when register-text contains both inflection patterns
